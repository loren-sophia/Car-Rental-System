...
def initialize_db():
    ...
    cursor.executescript("""
        CREATE TABLE IF NOT EXISTS vehicles (
            ...
            vehicle_type TEXT NOT NULL CHECK(vehicle_type IN ('Sedan','SUV','Pickup','Van','Convertible','Hatchback')),
            rate_per_day REAL NOT NULL CHECK(rate_per_day > 0),
            status TEXT NOT NULL DEFAULT 'available' CHECK(status IN ('available','reserved','rented','maintenance'))
        );
        ...
        CREATE TABLE IF NOT EXISTS reservations (
            ...
            status TEXT NOT NULL DEFAULT 'pending' CHECK(status IN ('pending','active','completed','cancelled','converted'))
        );
        ...
        CREATE TABLE IF NOT EXISTS rentals (
            ...
            end_date TEXT NOT NULL,
            total_cost REAL,
            status TEXT NOT NULL DEFAULT 'active' CHECK(status IN ('active','completed','late')),
    ...
    # ── Migrations: add columns/statuses safely on existing DBs ──────────────
    try:
